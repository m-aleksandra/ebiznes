// ✅ cypress/e2e/cart_hardcoded.cy.js

describe("Cart flow with hardcoded cartId = 1", () => {
  beforeEach(() => {
  ...
  it("2. Adds first product to cart", () => {
    cy.get("button.button").contains("Add to cart").first().click();
  });
  ...
  it("12. Add multiple products to cart", () => {
    cy.get("button.button").contains("Add to cart").first().click();
    cy.get("button.button").contains("Add to cart").eq(1).click();
    cy.contains("Cart").click();
  ...
  it("14. Total updates when adding products", () => {
    cy.get("button.button").contains("Add to cart").first().click();
    cy.get("button.button").contains("Add to cart").eq(2).click();
    cy.contains("Cart").click();
    cy.contains("Total:").should("exist");
  });
  ...
  it("18. Add to cart buttons exist", () => {
    cy.get("button.button").contains("Add to cart").should("exist");
  });
